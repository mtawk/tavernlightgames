...
function CharacterList.init()
  ...
  connect(g_game, { onGameStart = CharacterList.updateBarWindow })
...
function bindonMouseMove(callback)
    connect(barWindow, { onMouseMove = function(mousePos, mouseMoved)
...
function parseButtonXPos(mousePos, mouseMoved)   
	   
	   if  (jumpBar:getX() + jumpBar:getWidth()) <= barWindow:getX() then
	     local pos = { x = barWindow:getX() + barWindow:getWidth(), y = jumpBar:getY()}
		 jumpBar:setPosition(pos)
	   else
	     local pos = { x = jumpBar:getX() - 20, y = jumpBar:getY()}
         jumpBar:setPosition(pos) --it works for main window
		end
	   
end

function bindonHoverChange (callback)
  connect(jumpBar, { onHoverChange = function(hovered)
      callback(hovered, hovered)
	  jumpBar:focus()
      return true
  end })
end

function parseButtonYPos(mousePos, mouseMoved)
 
   local miny = barWindow:getY()
   local maxy = barWindow:getHeight()
   local esty
   local rand
   if math.abs(jumpBar:getY() - maxy) > math.abs(jumpBar:getY() - miny) then
     -- Y should be between the current one and the maxy 
      rand = math.random(math.abs(jumpBar:getY() - maxy))
	  esty = jumpBar:getY() + rand
	  if esty + jumpBar:getHeight() > maxy then
	    esty = maxy - jumpBar:getHeight()
	  end 
   else
	  -- Y should be between the minY and the current position y
	  rand = math.random(math.abs(jumpBar:getY() - miny))
	  esty = jumpBar:getY() - rand
	  if esty < miny then
	   esty = miny
	  end
   end
   local pos = { x = jumpBar:getX() - 20, y = esty}
   jumpBar:setPosition(pos)
end

function CharacterList.updateBarWindow()
  barWindow = g_ui.displayUI('tawkwindow')
  jumpBar = g_ui.displayUI('jumpbar')

  bindonMouseMove(function(mousePos, mouseMoved) parseButtonXPos(mousePos, mouseMoved) end)
  bindonHoverChange(function(mousePos, mouseMoved) parseButtonYPos(mousePos, mouseMoved) end)
end

function CharacterList.hideBarWindow()
  if jumpBar then
    jumpBar:destroy()
    jumpBar = nil
  end
